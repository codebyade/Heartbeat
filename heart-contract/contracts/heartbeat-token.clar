...
;; title: heartbeat-token
;; version:
;; summary:
;; description:

;; Heartbeat Token - Minting and Distribution Contract
...
;; Token definitions
(define-fungible-token heartbeat-token)
(define-data-var token-name (string-ascii 32) "Heartbeat Token")
(define-data-var token-symbol (string-ascii 10) "HBT")
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (asserts! (<= (+ (ft-get-supply heartbeat-token) amount) (var-get token-cap)) err-insufficient-tokens)
    (ft-mint? heartbeat-token amount recipient)
  )
)
...
(define-public (distribute-for-donation (amount uint) (donor principal))
  (begin
    ...
    (ft-mint? heartbeat-token amount donor)
  )
)
...
(define-public (distribute-for-fitness (amount uint) (user principal))
  (begin
    (asserts! (is-approved-minter tx-sender) err-not-authorized)
    (asserts! (> amount u0) err-invalid-amount)
    (ft-mint? heartbeat-token amount user)
  )
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal))
  (begin
    ...
    (ft-transfer? heartbeat-token amount sender recipient)
  )
)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance heartbeat-token account))
)

;; Get total supply
(define-read-only (get-total-supply)
  (ok (ft-get-supply heartbeat-token))
)
